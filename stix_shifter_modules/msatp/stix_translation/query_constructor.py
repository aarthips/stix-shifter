    ...
    def __eval_comparison_exp(self, expression):
        ...
        if 'created' in stix_field:
            ...
            if comparator == self.comparator_lookup.get(ComparisonComparators.Like):
                ...
                raise SearchFeatureNotSupportedError("Comparator {comparator} unsupported for Directory Path, "
                                                     "use only LIKE operator".format(comparator=comparator))
        else:
